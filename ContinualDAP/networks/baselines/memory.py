...
class Buffer:
    ...
    def add_from_loader(self,model,train_dataloader):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            labels = batch['labels']
            # task = batch['task']
            # task  = batch.get('task',torch.tensor(0, dtype=labels.dtype, device=labels.device).unsqueeze_(0))
            # outputs = model(batch, output_hidden_states=True)
            batch_tmp = {}
            batch_tmp['input_ids'] = input_ids
            batch_tmp['inputs_ori_ids'] = input_ids
            batch_tmp['attention_mask'] = attention_mask
            batch_tmp['labels'] = labels
            # outputs = model.module.model(input_ids=input_ids, labels=labels, attention_mask=attention_mask, output_hidden_states=True)
            outputs = model(batch_tmp, derpp_replay=True)

            logits = outputs.hidden_states[-1]

            # print('before gather_input')
            # print(f"Rank {dist.get_rank()} input_ids shape: {input_ids.shape}", flush=True)
            # gather_input = self.gather_by_cat(input_ids)
            # print('before gather_att')
            # print(f"Rank {dist.get_rank()} attention_mask shape: {attention_mask.shape}", flush=True)
            # gather_att = self.gather_by_cat(attention_mask)
            # print('before gather_labels')
            # print(f"Rank {dist.get_rank()} labels shape: {labels.shape}", flush=True)
            # gather_labels = self.gather_by_cat(labels)
            # print('before gather_logits')
            # print(f"Rank {dist.get_rank()} logits shape: {logits.shape}", flush=True)
            # gather_logits = self.gather_by_cat(logits)

            # make tensor shape : [batch_size, 1]
            task_id = torch.zeros(input_ids.size(0), dtype=input_ids.dtype, device=input_ids.device).fill_(self.args.pt_task)
            ...
            if (step+2)*len(batch['input_ids']) > self.args.replay_sample_per_task:
                break
