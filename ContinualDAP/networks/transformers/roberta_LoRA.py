...
class LoRARobertaSelfAttention(am.MultiTaskModule):
    def __init__(self, config, position_embedding_type=None):
        ...
        self.query = LoRAPiggybackLinear(
            config.hidden_size, self.all_head_size, config.lora_r, lora_alpha=config.lora_alpha, lora_dropout=config.lora_dropout, config=config)
...
class LoRARobertaSelfOutput(am.MultiTaskModule):
    def __init__(self, config):
        ...
        self.dense = LoRAPiggybackLinear(
            config.hidden_size, config.hidden_size, config.lora_r, lora_alpha=config.lora_alpha, lora_dropout=config.lora_dropout, config=config)
        self.LayerNorm = nn.LayerNorm(
            config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class LoRARobertaIntermediate(am.MultiTaskModule):
    def __init__(self, config):
        super().__init__()
        self.dense = LoRAPiggybackLinear(
            config.hidden_size, config.intermediate_size, config.lora_r, lora_alpha=config.lora_alpha, lora_dropout=config.lora_dropout, config=config)
...
class LoRARobertaOutput(am.MultiTaskModule):
    def __init__(self, config):
        super().__init__()
        self.dense = LoRAPiggybackLinear(
            config.intermediate_size, config.hidden_size, config.lora_r, lora_alpha=config.lora_alpha, lora_dropout=config.lora_dropout, config=config)
        self.LayerNorm = nn.LayerNorm(
            config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class LoRARobertaEncoder(am.MultiTaskModule):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([LoRARobertaLayer(config)
...
class LoRARobertaModel(am.MultiTaskModule):
    ...
    def __init__(self, config, args, add_pooling_layer=True):
        ...
        self.embeddings = LoRARobertaEmbeddings(config)
    ...
    def adaptation(self, num_class, task_label):
        ...
        # if self.config has attribute 'finetune_type'
        if hasattr(self.config, 'finetune_type'):
            if 'ModelWise_signMerge' in self.config.finetune_type:
                self.config.mergeratio_modelwise = nn.ParameterDict()
                for task_label in range(self.config.pt_task+1):
                    if task_label == self.config.ft_task:
                        if 'Residual' not in self.config.finetune_type:
                            self.config.mergeratio_modelwise[str(task_label)] = nn.Parameter(torch.tensor(1.0).cuda())
                        else:
                            self.config.mergeratio_modelwise[str(task_label)] = nn.Parameter(torch.tensor(0.0).cuda())
                    else:
                        self.config.mergeratio_modelwise[str(task_label)] = nn.Parameter(torch.tensor(0.0).cuda())
            if 'ModelWise' in self.config.finetune_type and 'effi' in self.config.finetune_type:
                self.config.mergeratios_modelwise = nn.Parameter(torch.ones(self.config.pt_task+1).cuda())
                # set one for mergeratios_modelwise[self.config.ft_task], else zero
                with torch.no_grad():
                    self.config.mergeratios_modelwise.fill_(0.0)
                    self.config.mergeratios_modelwise[self.config.ft_task] = 1.0
...
class LoRARobertaForLoRAEndtask(am.MultiTaskModule):
    def __init__(self, config, args, initial_out_features):
        ...
        self.roberta = LoRARobertaModel(config, args, add_pooling_layer=False)
        self.classifier = LoRARobertaClassificationHead(
            config, initial_out_features)
