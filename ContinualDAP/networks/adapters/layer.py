...
class AdapterLayer(AdapterLayerBase):
    def __init__(self, location_key: str, config, args):
        super().__init__()
    ...
    def add_fusion_layer(self, adapter_names: Union[List, str]):
        ...
        adapter_names = adapter_names if isinstance(
            adapter_names, list) else adapter_names.split(",")
    ...
    def delete_fusion_layer(self, adapter_names: Union[List, str]):
        adapter_names = adapter_names if isinstance(
            adapter_names, str) else ",".join(adapter_names)
    ...
    def adapter_stack(self, adapter_setup: Stack, hidden_states, input_tensor, layer_type, down_mask, up_mask, layer_norm, lvl=0):
        """
        ...
        for i, adapter_stack_layer in enumerate(adapter_setup):
            ...
            # Case 5: We have a single adapter which is part of this module -> forward pass
                ...
                hidden_states, _, up = adapter_layer(
                    hidden_states, residual_input=residual, layer_type=layer_type, down_mask=down_mask, up_mask=up_mask)
    ...
    def adapter_split(self, adapter_setup: Split, hidden_states, input_tensor, layer_norm, lvl=0):
        ...
        for i, adapter_block in enumerate(adapter_setup):
            ...
            # Case 4: We have a single adapter which is part of this module -> forward pass
                ...
                adapter_layer = self.adapters[adapter_block]
                split_hidden_states[i], _, _ = adapter_layer(
                    split_hidden_states[i], residual_input=split_residual[i])
        ...
        for i, child in enumerate(adapter_setup):
            ...
            if isinstance(child, Stack):
                child_hidden_states, _, _ = self.adapter_stack(
                    child,
                    hidden_states[i *
                                  orig_batch_size: (i + 1) * orig_batch_size],
                    input_tensor[i *
                                 orig_batch_size: (i + 1) * orig_batch_size],
                    layer_norm,
                    lvl=lvl + 1,
    ...
    def adapter_layer_forward(self, hidden_states, input_tensor, layer_type, down_mask, up_mask, layer_norm):
        """
        ...
        if not skip_adapters and (len(set(self.adapters.keys()) & adapter_setup.flatten()) > 0):
            ...
            if isinstance(adapter_setup, Stack):
                hidden_states, _, input_tensor = self.adapter_stack(
                    adapter_setup, hidden_states, input_tensor, layer_type, down_mask, up_mask, layer_norm
                )
            ...
            hidden_states = last_adapter.post_forward(
                hidden_states, input_hidden_states, input_tensor, layer_norm)
    ...
    def forward(self, hidden_states, input_tensor, layer_type, down_mask, up_mask, layer_norm):
        return self.adapter_layer_forward(hidden_states, input_tensor, layer_type, down_mask, up_mask, layer_norm)
