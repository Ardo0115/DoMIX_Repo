    ...
    for i in tqdm(range(num_training_steps)):
        ...
        for n in to_optimize:
            if n.endswith(f'.{moving_idx}'):
                ...
                norm_diff = ((B2P_i.norm(dim=0) - B2.norm(dim=0))**2).sum()
                reg_term = 0.1 * norm_diff
                loss += reg_term

                P_i_inv = torch.inverse(P_i)
                P_i_invA2 = P_i_inv @ A2
                norm_diff = ((P_i_invA2.norm(dim=1) - A2.norm(dim=1))**2).sum()
                reg_term = 0.1 * norm_diff
                loss += reg_term

                B2P_i_norm = B2P_i / (B2P_i.norm(dim=0, keepdim=True) + 1e-8)
                B_1_norm = B1 / (B1.norm(dim=0, keepdim=True) + 1e-8)

                cos_sim_matrix = B2P_i_norm.T @ B_1_norm
...
if __name__ == '__main__':
    ...
    # make directory f'./New_lora_transform_BZeroCossim{ref_dir[:3].upper()}/seq0/Nonesamples/lora_pbatch16_ngpu4_lr1e-04_r8 if not exist
    if not os.path.exists(f'./New_lora_transform_BZeroCossim{ref_dir[:3].upper()}/seq0/Nonesamples/lora_pbatch16_ngpu4_lr1e-04_r8'):
        os.makedirs(f'./New_lora_transform_BZeroCossim{ref_dir[:3].upper()}/seq0/Nonesamples/lora_pbatch16_ngpu4_lr1e-04_r8')
    model_tomerge1 = f'./seq0/Nonesamples/lora_pbatch16_ngpu4_lr1e-04_r8/{ref_dir}/'
    ...
    os.system(f'cp -a {model_tomerge1} ./New_lora_transform_BZeroCossim{ref_dir[:3].upper()}/seq0/Nonesamples/lora_pbatch16_ngpu4_lr1e-04_r8')

    
    # copy directory of model_tomerge1
    # os.system(f'rsync -av --exclude="model.pt" ./seq0/Nonesamples/lora_pbatch16_ngpu4_lr1e-04_r8/camera_unsup_{splitted_arch} ./New_lora_transform_BZeroCossim{ref_dir[:3].upper()}/seq0/Nonesamples/lora_pbatch16_ngpu4_lr1e-04_r8')

    for i in range(6):
        if i == args.fixed_model:
            continue
        model_tomerge2 = f'./seq0/Nonesamples/lora_pbatch16_ngpu4_lr1e-04_r8/{dirs[i]}/'
        ...
        torch.save(model_state1, f'./New_lora_transform_BZeroCossim{ref_dir[:3].upper()}/seq0/Nonesamples/lora_pbatch16_ngpu4_lr1e-04_r8/{ref_dir}/model.pt')
    
    os.system(f'mv ./New_lora_transform_BZeroCossim{ref_dir[:3].upper()}/seq0/Nonesamples/lora_pbatch16_ngpu4_lr1e-04_r8/{ref_dir} ./New_lora_transform_BZeroCossim{ref_dir[:3].upper()}/seq0/Nonesamples/lora_pbatch16_ngpu4_lr1e-04_r8/camera_unsup_{splitted_arch}')
