...
class Appr(object):
    ...
    def train(self, model, accelerator, train_dataset, train_loader, train_loader_subset, train_loader_subset_dataset):
        ...
        model, optimizer, train_loader, train_loader_subset = accelerator.prepare(
            model, optimizer, train_loader, train_loader_subset)
        ...
        completed_steps = 0
        # This will be used by CLMOE if we choose 'auto_encoder' as the route type.
        global_step = 0
                ...
                for epoch in range(self.args.num_train_epochs):
                    ...
                    for step, batch in enumerate(train_loader):
                        if 'derpp' in self.args.baseline \
                            ...
                            self, model, outputs, replay_outputs, replay_batch_tmp = compute_loss.compute(
                                self, model, batch, head_impt, intermediate_impt, output_impt, self_fisher, mask_pre, train_loader, step, accelerator, buffer=buffer)
                        else:
                        ...
                        if 'derpp' in self.args.baseline \
                            ...
                            model = compute_gradient.compute(
                                self, model, head_impt, intermediate_impt, output_impt, batch, loss, buffer, mask_back, outputs, epoch, step, accelerator, replay_outputs, replay_batch_tmp)
                        else:
                            model = compute_gradient.compute(
    ...
    # Assume no parralelism
    def eval(self, model, accelerator, eval_dataset, eval_loader):
        # Prepare everything with our `accelerator`.
        model, eval_dataset, eval_loader = accelerator.prepare(
            model, eval_dataset, eval_loader)
        num_update_steps_per_epoch = len(eval_loader)
        self.args.num_eval_epochs = 1
        self.args.max_eval_steps = self.args.num_eval_epochs * num_update_steps_per_epoch

        val_loss = 0

        try:
            for epoch in range(self.args.num_eval_epochs):
                # break
                model.eval()
                for step, batch in tqdm(enumerate(eval_loader)):

                    self, model, outputs = compute_loss.compute(
                        self, model, batch, None, None, None, None, None, None, 1, None)
                    loss = outputs.loss  # loss 1
                    
                    # Compute running average of val_loss
                    val_loss = (val_loss * step + loss.item()) / (step + 1)
