...
def compute(self, model, train_loader_subset, self_fisher, mask_pre, buffer, accelerator):
    ...
    if accelerator.is_main_process:
        ...
        if 'prompt' in self.args.baseline or 'l2p' in self.args.baseline:
            torch.save(unwrapped_model.model.keys,
                       os.path.join(self.args.output_dir,  'keys'))
            torch.save(unwrapped_model.model.prompt_pool,
                       os.path.join(self.args.output_dir, 'prompt_pool'))

    if 'ewc' in self.args.baseline:
        ewc.compute(train_loader_subset, model,
                    self_fisher, accelerator, self.args)
        ...
        hat.compute(model, accelerator, mask_pre,
                    mask, self.get_view_for, self.args)
