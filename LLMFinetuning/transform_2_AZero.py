...
def total_objective_function(A_dict, B1_dict, A1_dict, B_ref_dict, A_ref_dict):
    ...
    for key in A_dict.keys():
        ...
        norm_diff = ((P_iA1.norm(dim=1) - A1.norm(dim=1)) ** 2).sum()
        reg_term = 0.1 * norm_diff  # Regularization coefficient (adjust as needed)
        total_loss += reg_term

        P_i_inv = torch.inverse(P_i)
        BP_i_inv = B1 @ P_i_inv
        norm_diff = ((BP_i_inv.norm(dim=0) - B1.norm(dim=0)) ** 2).sum()
        reg_term = 0.1 * norm_diff  # Regularization coefficient (adjust as needed)
        total_loss += reg_term
...
save_dir = '/home/ardo0115/workspace/Subspace-Tuning/CR_MR/finetune/Rank16_Sched_InitId_AzeroCossim_transformed_from-math-to-Numerical_epoch2'
