...
for key in lora_B_keys:
    ...
    # Initialize A_i with identity matrix
    A_i = torch.eye(r, device=device, requires_grad=True)
    A_dict[key] = A_i
...
def total_objective_function(A_dict, B1_dict, B_ref_dict, A1_dict, A_ref_dict):
    ...
    for key in A_dict.keys():
        ...
        loss_i = torch.norm(BP_i - B_ref, p='fro')**2
        loss_i += torch.norm(A_ref - P_i_inv_A1, p='fro')**2
        total_loss += loss_i
    return total_loss
...
for key in A_dict.keys():
    ...
    A1 = A1_dict[key_A]
    A_ref = A_ref_dict[key_A]
    objective_optimal = torch.norm((B1 @ P_i) - B_ref, 'fro')**2
    objective_optimal += torch.norm(A_ref - (torch.inverse(P_i) @ A1), 'fro')**2
    ...
    objective_closed = torch.norm((B1 @ P_closed) - B_ref, 'fro')**2
    objective_closed += torch.norm(A_ref - (torch.inverse(P_closed) @ A1), 'fro')**2
...
save_dir = '/home/ardo0115/workspace/Subspace-Tuning/CR_MR/finetune/ABclose_transformed_from-lora-to-Numerical-fromEye'
...
plt.grid(True)
plt.savefig('optimization_loss_curve_eye.png')
plt.show()
