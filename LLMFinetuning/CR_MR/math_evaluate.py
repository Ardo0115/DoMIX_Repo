...
def parse_args():
    ...
    parser.add_argument('--dataset', choices=['AddSub', 'MultiArith', 'SingleEq', 'gsm8k', 'AQuA', 'SVAMP', 'MAWPS'],
                        required=True)
    parser.add_argument('--model', choices=['LLaMA-7B', 'BLOOM-7B', 'GPT-j-6B', 'Llama-3-8B'], required=True)
    parser.add_argument('--adapter', choices=['LoRA', 'AdapterP', 'AdapterH', 'Parallel', 'Prefix', 'DoRA'],
                        required=True)
...
def load_model(args) -> tuple:
    ...
    if device == "cuda":
        ...
        # if 'lora-merging' in lora_weights:
        #     config = LoraConfig(
        #         r=8,
        #         lora_alpha=16,
        ...
        #     model.to("cuda")
        #     to_load_state_dict = torch.load(os.path.join(lora_weights, "adapter_model.bin"))
        #     # Check all keys are present
        #     for key in to_load_state_dict:
        #         if key not in model.state_dict():
        #             print(f"Key {key} not found in model")
        #             raise ValueError(f"Key {key} not found in model")
        #     model.load_state_dict(to_load_state_dict, strict=False)
        # else:
