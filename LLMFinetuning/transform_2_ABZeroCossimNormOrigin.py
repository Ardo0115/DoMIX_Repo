...
def total_objective_function(A_dict, B1_dict, A1_dict, B_ref_dict, A_ref_dict):
    ...
    for key in A_dict.keys():
        ...
        P_iA_norm = P_iA1 / (P_iA1.norm(dim=1, keepdim=True) + 1e-8)
        A_ref_norm = A_ref / (A_ref.norm(dim=1, keepdim=True) + 1e-8)

        cos_sim_matrix = P_iA_norm @ A_ref_norm.T
        
        # Square the cosine similarity matrix and sum all elements to get the total loss for this key
        loss_i = (cos_sim_matrix ** 2).sum()

        BP_i_inv_norm = BP_i_inv / (BP_i_inv.norm(dim=0, keepdim=True) + 1e-8)
        B_ref_norm = B_ref / (B_ref.norm(dim=0, keepdim=True) + 1e-8)

        cos_sim_matrix = BP_i_inv_norm.T @ B_ref_norm
...
for key in A_dict.keys():
    ...
    original_norm_B1 = moving_lora[key].norm(dim=0, keepdim=True)
    moving_lora[key] = (B1 @ torch.inverse(P_i)).detach().cpu()
    # normalize to original norm
    moving_lora[key] = moving_lora[key] / (moving_lora[key].norm(dim=0, keepdim=True) + 1e-8) * original_norm_B1
    key_A = key.replace('lora_B.weight', 'lora_A.weight')

    original_norm_A1 = moving_lora[key_A].norm(dim=1, keepdim=True)
    moving_lora[key_A] = (P_i @ moving_lora[key_A].to(device)).detach().cpu()
    # normalize to original norm
    moving_lora[key_A] = moving_lora[key_A] / (moving_lora[key_A].norm(dim=1, keepdim=True) + 1e-8) * original_norm_A1

# Step 5: Save the transformed state_dict (moving_lora)
save_dir = '/home/ardo0115/workspace/Subspace-Tuning/CR_MR/finetune/Rank16_Sched_InitId_ABzeroCossimNormOrigin_transformed_from-lora-to-Numerical_epoch2'
